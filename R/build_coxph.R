  ...
  if (!is.null(pred_time) || !is.null(pred_survival_rate)) {
    ...
    if (!is.null(pred_time)) {
      ...
      if (x$clean_status_col %in% colnames(ret)) {
        ...
        ret <- ret %>% dplyr::mutate(note = if_else(current_survival_time > survival_time_for_prediction, "Before the end time", if_else(!!rlang::sym(x$clean_status_col), "After the event time", note)))
      }
      else {
        ...
        ret <- ret %>% dplyr::mutate(note = if_else(current_survival_time > survival_time_for_prediction, "Before the end time", note))
      }
