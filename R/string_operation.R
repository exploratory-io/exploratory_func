...
do_tokenize_icu <- function(df, text_col, token = "word", keep_cols = FALSE,
  ...
  df <- dplyr::mutate(df, !!rlang::sym(doc_id) := dplyr::row_number())
...
do_tokenize <- function(df, text, token = "words", keep_cols = FALSE,
  ...
  if (with_sentence_id || token == "sentences") {
    ...
    tryCatch({
      ...
      if (any(stringr::str_detect(e$message, "invalid UTF-8 byte sequence detected"))) {
...
do_ngram <- function(df, token, sentence, document, maxn=2, sep="_"){
  ...
  for(n in seq(maxn)[-1]){
    ...
    lead_expr <- rlang::expr(dplyr::lead(!!rlang::sym(token_col), !!as.integer(n-1)))
    ...
    str_c_expr <- rlang::expr(stringr::str_c(!!rlang::sym(as.character(prev_cname)), !!rlang::sym(as.character(cname)), sep = !!sep))

    # execute the expressions
    grouped <- (grouped %>%
              dplyr::mutate(!!as.character(cname) := !!lead_expr) %>%
              dplyr::mutate(!!as.character(cname) := !!str_c_expr)
              )
