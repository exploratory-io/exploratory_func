...
build_lda <- function(df, subject, key, value = NULL,
  ...
  build_lda_each <- function(df){
    ...
    if(output == "model"){
      ...
      phi_melt <- (lda$get_word_vectors() + topic_word_prior) %>%
        t %>%
        text2vec::normalize("l1") %>%
        reshape2::melt()
      # rename columns avoiding conflict among grouped columns
      c_names <- avoid_conflict(grouped_column, c("topic", "term", "value"))
      ...
      model <- lda$get_fitted_LDA_model()
      theta_mat = (model$document_topic_distr + doc_topic_prior) %>%
        t %>%
        text2vec::normalize("l1")
      rownames(theta_mat) <- documents_name
      theta_melt <- theta_mat %>%
        reshape2::melt()
      # rename columns avoiding conflict among grouped columns
      c_names <- avoid_conflict(grouped_column, c("document", "topic", "value"))
  ...
  ret <- df %>%
    dplyr::do_(.dots=setNames(list(~build_lda_each(.), ~(.)), c(model_column, source_column)))
  if(output != "model"){
    ret <- tidyr::unnest_(ret, model_column)
  }
  ret
}
