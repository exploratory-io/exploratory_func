...
#' Calculate distances from zoo object
calculate_distances_from_zoo <- function(zoo_data, target_market, id = "id", warping_limit, matches, dtw_emphasis){
  row <- 1
  all_markets <- names(zoo_data)
  distances <- data.frame(matrix(nrow=length(all_markets), ncol=5))
  names(distances) <- c(id, "market", "distance", "correlation", "Length")
  for (ThatMarket in all_markets){
    ...
    ref <- zoo_data[, target_market]
    
    index_without_na <- !(is.na(test) | is.na(ref))
    
    # remove index where either has NA
    test <- test[index_without_na]
    ref <- ref[index_without_na]
    ...
    if (target_market != ThatMarket & messages==0 & var(ref)>0 & length(test)>2*warping_limit){
      if (dtw_emphasis>0){
        dist <- dtw::dtw(test, ref, window.type=dtw::sakoeChibaWindow, window.size=warping_limit)$distance / abs(sum(test))
      } else{
        dist <- 0
      }
      distances[row, "correlation"] <- cor(test, ref)
      distances[row, "distance"] <- dist
      distances[row, "Skip"] <- FALSE
    } else{
      distances[row, "Skip"] <- TRUE
      distances[row, "distance"] <- NA
...
calculate_distances <- function(all_markets, data, id, i, warping_limit, matches, dtw_emphasis){
  ...
  for (j in 1:length(all_markets)){
    ...
    if (ThisMarket != ThatMarket & messages==0 & var(ref)>0 & length(test)>2*warping_limit){
      if (dtw_emphasis>0){
        dist <- dtw(test, ref, window.type=dtw::sakoeChibaWindow, window.size=warping_limit)$distance / abs(sum(test))
      } else{
        dist <- 0
      }
      distances[row, "Correlation"] <- cor(test, ref)
...
best_matches <- function(data=NULL, id_variable=NULL, target_value = NULL, date_variable=NULL, matching_variable=NULL, warping_limit=1, parallel=TRUE, start_match_period=NULL, end_match_period=NULL, matches=5, dtw_emphasis=1){
  ...
  calculate_distances_with_trycatch <- function(...){
  ...
  if (parallel==FALSE){
    if(!is.null(target_value)){
      target_index <- which(all_markets == target_value)
      # validation of target value
      if(length(target_index) == 0){
        stop(paste0(c(target_index, " is not in markets")))
      }
      if(length(target_index) > 1){
        stop(paste0(c(target_index, " is duplicated in markets")))
      }
      all_distances[[target_index]] <- calculate_distances_with_trycatch(all_markets, data, id_variable, target_index, warping_limit, matches, dtw_emphasis)
    } else {
      for (i in 1:length(all_markets)){
        cat(i)
        all_distances[[i]] <- calculate_distances_with_trycatch(all_markets, data, id_variable, i, warping_limit, matches, dtw_emphasis)
      } 
    }
    shortest_distances <- data.frame(data.table::rbindlist(all_distances))
  } else{
    ncore <- detectCores()-1
    ...
    shortest_distances <- data.frame(data.table::rbindlist(loop_result))
    stopImplicitCluster()
  }
...
best_matches_from_zoo <- function(zoo_data=NULL, target_value = NULL, warping_limit=1, parallel=TRUE, end_match_period=NULL, matches=5, dtw_emphasis=1){
  ...
  ## filter the dates
  data <- zoo_data[zoo::index(zoo_data) < as.Date(end_match_period), ]
  ...
  if (parallel==FALSE){
    if(!is.null(target_value)){
      ...
      for (i in 1:length(all_markets)){
        cat(i)
        all_distances[[i]] <- calculate_distances_with_trycatch(data, target_market=names(data)[i], warping_limit = warping_limit, matches = matches, dtw_emphasis = dtw_emphasis)
      } 
    }
    shortest_distances <- data.frame(data.table::rbindlist(all_distances))
  } else{
    ...
    loop_result <- foreach(i=1:length(all_markets)) %dopar% {
      cat(i)
      calculate_distances_with_trycatch(data, target_value=names(data)[i],  warping_limit = warping_limit, matches = matches, dtw_emphasis = dtw_emphasis)
    }
    shortest_distances <- data.frame(data.table::rbindlist(loop_result))
    stopImplicitCluster()
  }
  ...
  ### Return the results
  object <- list(BestMatches=shortest_distances, Data=data)
  class(object) <- "matched_market"
  return (object)
}
