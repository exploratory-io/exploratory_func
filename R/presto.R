#' @export
queryPresto <- function(host, port, username, password, schema, catalog, numOfRows = -1, query, timezone = "UTC"){

  # read stored password
  pass = saveOrReadPassword("presto", username, password)
  drv <- RPresto::Presto()
  conn <- RPresto::dbConnect(drv, user = username,
                                password = pass, host = host, port = port, schema = schema, catalog = catalog, session.timezone = timezone)
