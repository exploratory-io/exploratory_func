...
test_that("Random Forest: Group with fewer samples should not be skipped in the result.", {
  ...
  res <- model_df %>% rf_partial_dependence() # Just make sure edarf works without error, handling Stores with no significant variable well.

  # Test edarf with no group_by to handle the case where there is no significant variagle.
  model_df <- df %>% dplyr::filter(`Store` == "Napa") %>% calc_feature_imp(`ChurnedWithinYear`, `Region`, `Gender`, `Age`, `JobField`, `MaritalStatus`, `ContactType`, `MemberCard`, `MonthlySpending`, `BeerTastingClass`, `CookingClass`, `FlowerArrangementClass`, `PaperCraftClass`, `WineClass`, `DistanceFromStore`, `NumberOfStoresVisited`, `StoreVisitsPerMonth`, `NewsLetterOpened`, `WebSiteEngagement`, `SurveyOverallRating`, `FacebookPageLikes`, `EnvironmentCareness`, smote = TRUE, importance_measure = "permutation", with_boruta = TRUE)
  res <- model_df %>% rf_evaluation_training_and_test()
  res <- model_df %>% rf_partial_dependence() # Just make sure edarf works without error, handling no significant variable case well.
})
