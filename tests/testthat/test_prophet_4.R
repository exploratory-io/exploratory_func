...
test_that("do_prophet with holiday country", {
  ...
  # Test with FR (France) holidays. JP was removed from prophet 1.1.7's generated_holidays.
  ret <- raw_data %>%
    do_prophet(timestamp, data, 10, time_unit = "day", holiday_country_names=c("US","FR"))
  expect_true("Christmas Day" %in% names(ret))
})
