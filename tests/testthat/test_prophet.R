...
test_that("do_prophet test mode with minute as time units", {
  if (Sys.info()["machine"] == "x86") { # need to be shorter on win 32bit to avoid memory error.
    ts <- seq(as.POSIXct("2010-01-01:00:00:00"), as.POSIXct("2010-01-3:00:00"), by="min")
  }
  else {
  ...
  expect_true(!is.na(ret$forecasted_value[[length(ret$forecasted_value)]]))
  # verify that daily, weekly is enabled to test #9361.
  if (Sys.info()["machine"] == "x86") { # cannot make it long enough for weekly on win 32bit to avoid memory error.
