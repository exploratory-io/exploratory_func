...
test_that("prediction with target column name with space by build_lm.fast", {
  ...
  model_data <- build_lm.fast(test_data, `Carrier Name`, CANCELLED, CARRIER, DISTANCE)
  ret <- model_data %>% broom::augment(model)

  expect_true(nrow(ret) > 0)
  expect_equal(colnames(ret), c("Carrier.Name","CANCELLED","CARRIER","DISTANCE",".fitted",".se.fit",".resid",".hat",".sigma",".cooksd",".std.resid"))
})
