...
test_that("has_special_json_chars detects special characters", {
  ...
  expect_true(exploratory:::has_special_json_chars(c("col 1")))
  expect_true(exploratory:::has_special_json_chars(c("col,1")))
  expect_true(exploratory:::has_special_json_chars(c("col:1")))
  expect_true(exploratory:::has_special_json_chars(c("col(1)")))
  expect_true(exploratory:::has_special_json_chars(c("col@1")))
  ...
  expect_false(exploratory:::has_special_json_chars(c("col{1}", "col2")))
  expect_false(exploratory:::has_special_json_chars(c("col[1]")))
  expect_false(exploratory:::has_special_json_chars(c("col\"1")))
  expect_false(exploratory:::has_special_json_chars(c("col'1")))
  # Normal names without special characters
  expect_false(exploratory:::has_special_json_chars(c("col1", "col2")))
  expect_false(exploratory:::has_special_json_chars(c("abc_def", "xyz")))
  expect_false(exploratory:::has_special_json_chars(NULL))
  expect_false(exploratory:::has_special_json_chars(character(0)))
})

test_that("expand_lightgbm_metric_aliases expands aliases", {
  result <- exploratory:::expand_lightgbm_metric_aliases("mae")
  expect_true("mae" %in% result)
  expect_true("l1" %in% result)

  result2 <- exploratory:::expand_lightgbm_metric_aliases("mse")
  expect_true("mse" %in% result2)
  expect_true("l2" %in% result2)

  result3 <- exploratory:::expand_lightgbm_metric_aliases("rmse")
  expect_true("rmse" %in% result3)
  expect_true("l2_root" %in% result3)

  # Unknown metric should be returned as-is
  result4 <- exploratory:::expand_lightgbm_metric_aliases("unknown_metric")
  expect_equal(result4, "unknown_metric")

  # NULL input
  expect_null(exploratory:::expand_lightgbm_metric_aliases(NULL))
})

test_that("sanitize_lightgbm_colnames replaces special chars", {
  result <- exploratory:::sanitize_lightgbm_colnames(c("col{1}", "col[2]", "normal"))
  expect_false(any(grepl("[{}\\[\\]]", result)))
  expect_equal(result[3], "normal")

  # Multiple underscores should be collapsed
  result2 <- exploratory:::sanitize_lightgbm_colnames(c("a::b"))
  expect_false(grepl("__", result2))

  # Leading/trailing underscores removed
  result3 <- exploratory:::sanitize_lightgbm_colnames(c("{test}"))
  expect_false(grepl("^_|_$", result3))

  # Duplicate names made unique
  result4 <- exploratory:::sanitize_lightgbm_colnames(c("a b", "a b"))
  expect_equal(length(unique(result4)), 2)

  # NULL input
  expect_null(exploratory:::sanitize_lightgbm_colnames(NULL))
})
